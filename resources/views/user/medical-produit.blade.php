...
@section('userContent')
    ...
    <div class="marque-pro-body">
        <div class="mag-title text-start mb-2 d-flex align-items-center mt-4">
            @if (count($medPro) > 0)
                <h5 class="mt-2 me-2"><i class="fa-solid fa-right-long me-3"></i>Il y a {{ count($medPro) }} médical  produits</h5>
                <img class="mb-4" src="https://img.icons8.com/pastel-glyph/64/000000/hand-with-a-pill.png"/>
            @else
                <h5 class="mt-2 me-3"><i class="fa-solid fa-right-long me-3"></i>Aucun médical produits</h5>
            @endif
        </div>
        <div class="mt-0">
            <div class="row mt-4">
                @if (count($medPro) > 0)
                    @foreach ($medPro as $item)
